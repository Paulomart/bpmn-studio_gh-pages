...
describe('IdentityService.isDummyToken()', (): void => {
  ...
  it('Should return true for a dummy token', async (): Promise<void> => {
    const result = await identityService.isDummyToken(dummyToken);
    should(result).be.true();
  });

  it('Should return false for an internal token', async (): Promise<void> => {
    const result = await identityService.isDummyToken(internalToken);
    should(result).be.false();
  });

  it('Should return false for a real users token', async (): Promise<void> => {
    const result = await identityService.isDummyToken(sampleToken);
    should(result).be.false();
  });

  it('Should return false for an invalid token', async (): Promise<void> => {
    const result = await identityService.isDummyToken(sampleInvalidToken);
    should(result).be.false();
  });
