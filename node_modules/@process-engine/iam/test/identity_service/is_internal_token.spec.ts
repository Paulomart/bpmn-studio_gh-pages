...
describe('IdentityService.isInternalToken()', (): void => {
  ...
  it('Should return true for an internal token', async (): Promise<void> => {
    const result = await identityService.isInternalToken(internalToken);
    should(result).be.true();
  });

  it('Should return false for a dummy token', async (): Promise<void> => {
    const result = await identityService.isInternalToken(dummyToken);
    should(result).be.false();
  });

  it('Should return false for a real users token', async (): Promise<void> => {
    const result = await identityService.isInternalToken(sampleToken);
    should(result).be.false();
  });

  it('Should return false for an invalid token', async (): Promise<void> => {
    const result = await identityService.isInternalToken(sampleInvalidToken);
    should(result).be.false();
  });
