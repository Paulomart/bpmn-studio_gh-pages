export declare const messageParams: {
    correlationId: string;
    endEventId: string;
    flowNodeInstanceId: string;
    userTaskId: string;
    manualTaskId: string;
    messageReference: string;
    processInstanceId: string;
    processModelId: string;
    signalReference: string;
    emptyActivityId: string;
};
export declare const messagePaths: {
    boundaryEventTriggered: string;
    intermediateThrowEventTriggered: string;
    intermediateCatchEventReached: string;
    intermediateCatchEventFinished: string;
    activityReached: string;
    activityFinished: string;
    emptyActivityReached: string;
    emptyActivityFinished: string;
    userTaskReached: string;
    userTaskFinished: string;
    manualTaskReached: string;
    manualTaskFinished: string;
    processStarted: string;
    processEnded: string;
    processError: string;
    processTerminated: string;
    finishEmptyActivity: string;
    emptyActivityWithInstanceIdFinished: string;
    finishUserTask: string;
    userTaskWithInstanceIdFinished: string;
    finishManualTask: string;
    manualTaskWithInstanceIdFinished: string;
    endEventReached: string;
    messageEventReached: string;
    signalEventReached: string;
    terminateEndEventReached: string;
    processInstanceStarted: string;
    processInstanceEnded: string;
};
