define(["require", "exports", "@essential-projects/errors_ts", "@process-engine/consumer_api_contracts", "@process-engine/persistence_api.contracts", "./paginator", "./process_model_cache"], function (require, exports, errors_ts_1, consumer_api_contracts_1, persistence_api_contracts_1, paginator_1, ProcessModelCache) {
    "use strict";
    ...
    class EventService {
        ...
        async getEventsForProcessModel(identity, processModelId, offset = 0, limit = 0) {
        ...
        async getEventsForCorrelation(identity, correlationId, offset = 0, limit = 0) {
            const suspendedFlowNodeInstances = await this.flowNodeInstanceService.querySuspendedByCorrelation(correlationId);
            const eventList = await this.filterAndConvertEventList(identity, suspendedFlowNodeInstances, offset, limit);
            return eventList;
        }
        async getEventsForProcessModelInCorrelation(identity, processModelId, correlationId, offset = 0, limit = 0) {
        ...
        async triggerMessageEvent(identity, messageName, payload) {
            ...
            const messageEventName = consumer_api_contracts_1.Messages.EventAggregatorSettings.messagePaths.messageEventReached
                .replace(consumer_api_contracts_1.Messages.EventAggregatorSettings.messageParams.messageReference, messageName);
        ...
        async triggerSignalEvent(identity, signalName, payload) {
            ...
            const signalEventName = consumer_api_contracts_1.Messages.EventAggregatorSettings.messagePaths.signalEventReached
                .replace(consumer_api_contracts_1.Messages.EventAggregatorSettings.messageParams.signalReference, signalName);
        ...
        async convertToConsumerApiEvent(identity, suspendedFlowNode) {
        ...
        getEventDefinitionFromFlowNodeModel(flowNodeModel, eventType) {
            switch (eventType) {
                case consumer_api_contracts_1.DataModels.Events.EventType.messageEvent:
                    return flowNodeModel.messageEventDefinition.name;
                case consumer_api_contracts_1.DataModels.Events.EventType.signalEvent:
