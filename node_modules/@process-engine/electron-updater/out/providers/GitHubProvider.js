...
class GitHubProvider extends BaseGitHubProvider {
  ...
  async getLatestVersion() {
    ...
    try {
      if (this.updater.channel != null) {
        ...
        if (this.updater.channel.toLowerCase() == 'stable') {
          ...
          latestChannelRelease = feed.getElements("entry").find(element => {
            return element.element("link").attribute("href").includes(this.updater.channel);
          });
        }
    ...
    if (version == null && this.updater.channel != null) {
      const releaseUrl = (0, _main().newUrlFromBase)(`/repos${this.basePath}?per_page=100`, this.baseApiUrl);
      const headers = {
        accept: "application/json, */*"
      };
      const releaseList = await this.httpRequest(releaseUrl, headers, cancellationToken);

      if (releaseList == null) {
        throw (0, _builderUtilRuntime().newError)('Could not get release list.', "ERR_UPDATER_LATEST_VERSION_NOT_FOUND");
      }
