...
const logger = loggerhythm_1.Logger.createLogger('management_api:socket.io_endpoint:empty_activity');
class EmptyActivitySocketEndpoint extends http_node_1.BaseSocketEndpoint {
    ...
    async createSocketScopeNotifications(socketIoInstance) {
        const emptyActivityReachedSubscription = this.eventAggregator.subscribe(management_api_contracts_1.Messages.EventAggregatorSettings.messagePaths.emptyActivityReached, (emptyActivityWaitingMessage) => {
            socketIoInstance.emit(management_api_contracts_1.socketSettings.paths.emptyActivityWaiting, emptyActivityWaitingMessage);
        });
        const emptyActivityFinishedSubscription = this.eventAggregator.subscribe(management_api_contracts_1.Messages.EventAggregatorSettings.messagePaths.emptyActivityFinished, (emptyActivityFinishedMessage) => {
            socketIoInstance.emit(management_api_contracts_1.socketSettings.paths.emptyActivityFinished, emptyActivityFinishedMessage);
        });
    ...
    async createUserScopeNotifications(socket, identity) {
        ...
        const onEmptyActivityForIdentityWaitingSubscription = await this.notificationService.onEmptyActivityForIdentityWaiting(identity, (message) => {
            const eventToPublish = management_api_contracts_1.socketSettings.paths.emptyActivityForIdentityWaiting
        ...
        const onEmptyActivityForIdentityFinishedSubscription = await this.notificationService.onEmptyActivityForIdentityFinished(identity, (message) => {
            const eventToPublish = management_api_contracts_1.socketSettings.paths.emptyActivityForIdentityFinished
                .replace(management_api_contracts_1.socketSettings.pathParams.userId, identity.userId);
            socket.emit(eventToPublish, message);
        });
...
exports.EmptyActivitySocketEndpoint = EmptyActivitySocketEndpoint;
//# sourceMappingURL=empty_activity_socket_endpoint.js.map